...
def sequence_to_token_ids(sequence, tokenizer):
    ...
    return np.squeeze(np.array(token_ids))


def token_ids_to_pretrain(token_ids, pretrain):
    with torch.no_grad():
        emb = pretrain(token_ids)[0]

    return emb

def sequence_to_pretrain(sequence, tokenizer, pretrain):
    token_ids = torch.tensor(sequence_to_token_ids(sequence, tokenizer)).unsqueeze(0)
    emb = token_ids_to_pretrain(token_ids, pretrain)
